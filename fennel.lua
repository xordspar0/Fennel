package.preload["specials"] = package.preload["specials"] or function(...)
  local utils = require("utils")
  local parser = require("parser")
  local compiler = require("compiler")
  
  local SPECIALS = compiler.globalScope.specials
  local compilerScope = compiler.makeScope(compiler.globalScope)
  ...
  SPECIALS["fn"] = function(ast, scope, parent)
      ...
      if utils.rootOptions.useMetadata then
          ...
          local metaStr = ('require("%s").metadata'):
              format(utils.rootOptions.moduleName or "fennel")
  ...
  SPECIALS['doc'] = function(ast, scope, parent)
      assert(utils.rootOptions.useMetadata, "can't look up doc with metadata disabled.")
      ...
      if specialOrMacro then
          ...
          return ("print(require('%s').doc(%s, '%s'))")
              :format(utils.rootOptions.moduleName or "fennel", value, tostring(ast[2]))
      end
  end
  docSpecial("doc", {"x"},
  ...
  SPECIALS['quote'] = function(ast, scope, parent)
      ...
      while thisScope do
          ...
          if thisScope == compilerScope then runtime = false end
      end
  ...
  local function makeCompilerEnv(ast, scope, parent)
      return setmetatable({
              ...
              _AST = ast,
              _IS_COMPILER = true,
              _SPECIALS = compiler.globalScope.specials,
              _VARARG = utils.varg(),
              ...
              gensym = function()
                  return utils.sym(compiler.gensym(compiler.getMacroScope() or scope))
              end,
              ["list?"] = utils.isList,
              ...
              ["varg?"] = utils.isVarg,
              ["get-scope"] = function() return compiler.getMacroScope end,
              ["in-scope?"] = function(symbol)
                  compiler.assert(compiler.getMacroScope(), "must call from macro", ast)
                  return compiler.getMacroScope().manglings[tostring(symbol)]
              end,
              ["macroexpand"] = function(form)
                  compiler.assert(compiler.getMacroScope(), "must call from macro", ast)
                  return compiler.macroexpand(form, compiler.getMacroScope())
              end}, { __index = _ENV or _G })
  end
  ...
  local function loadMacros(modname, ast, scope, parent)
      ...
      return compiler.dofileFennel(filename,
                                   { env = env, allowedGlobals = globals,
                                     useMetadata = utils.rootOptions.useMetadata,
                                     scope = compilerScope })
  end
  ...
  SPECIALS['include'] = function(ast, scope, parent, opts)
      ...
      -- Check cache
      if utils.rootScope.includes[mod] then return utils.rootScope.includes[mod] end
      ...
      for i, v in ipairs(tempChunk) do table.insert(utils.rootChunk, i, v) end
  
      -- For fnl source, compile subChunk AFTER splicing into start of rootChunk.
      if isFennel then
          local subscope = compiler.makeScope(utils.rootScope.parent)
          if utils.rootOptions.requireAsInclude then
      ...
      utils.rootScope.includes[mod] = ret
      return ret
  end
  ...
  local function evalCompiler(ast, scope, parent)
      local luaSource = compiler.compile(ast, { scope = compiler.makeScope(compilerScope),
                                                useMetadata = utils.rootOptions.useMetadata })
...
package.preload["compiler"] = package.preload["compiler"] or function(...)
  local utils = require("utils")
  local parser = require("parser")
  local unpack = _G.unpack or table.unpack
  
  local globalScope, macroCurrentScope
  ...
  local function makeScope(parent)
      if not parent then parent = globalScope end
      return {
  ...
  local function assertCompile(condition, msg, ast)
      local override = utils.rootOptions and utils.rootOptions["assert-compile"]
      if override then
          local source = utils.rootOptions and utils.rootOptions.source
          -- don't make custom handlers deal with resetting root; it's error-prone
          if not condition and utils.resetRoot then utils.resetRoot() end
      ...
      if not condition then
          if utils.resetRoot then utils.resetRoot() end
  ...
  globalScope = makeScope()
  globalScope.vararg = true
  -- Save current macro scope; used by gensym, in-scope?, etc
  macroCurrentScope = globalScope
  local function getMacroScope() return macroCurrentScope end
  ...
  local function symbolToExpression(symbol, scope, isReference)
      ...
      if not isLocal then
          utils.rootScope.refedglobals[parts[1]] = true
      end
  ...
  local function requireInclude(ast, scope, parent, opts)
      ...
      return globalScope.specials['include'](ast, scope, parent, opts)
  end
  
  local function compileStream(strm, options)
      local opts = utils.copy(options)
      local oldGlobals = allowedGlobals
      utils:setResetRoot(utils.rootChunk, utils.rootScope, utils.rootOptions)
      allowedGlobals = opts.allowedGlobals
      if opts.indent == nil then opts.indent = '  ' end
      local scope = opts.scope or makeScope(globalScope)
      ...
      local chunk = {}
      utils.rootChunk, utils.rootScope, utils.rootOptions = chunk, scope, opts
      for i = 1, #vals do
  ...
  return {
      ...
      -- general functions and data:
      assert=assertCompile, traceback=traceback, getMacroScope=getMacroScope,
      metadata=makeMetadata(), globalScope=globalScope
  }
end
...
local function eval(str, options, ...)
  ...
  local loader = specials.loadCode(lua_source, env, (opts.filename or str))
  opts.filename = nil
  return loader(...)
end
...
compiler.dofileFennel = dofile_fennel
local path_table = {"./?.fnl", "./?/init.fnl"}
table.insert(path_table, utils.getenv("FENNEL_PATH"))
local function make_searcher(options)
  local function _0_(module_name)
    local opts = utils.copy(utils.rootOptions)
    local filename = specials.searchModule(module_name)
    for k, v in pairs((options or {})) do
      opts[k] = v
    end
    if filename then
      local function _1_(mod_name)
        return dofile_fennel(filename, opts, mod_name)
      end
      return _1_
    end
  end
...
do
  ...
  _ = nil
  local env = specials.makeCompilerEnv(nil, specials.compilerScope, {})
  local built_ins = eval(builtin_macros, {allowedGlobals = false, env = env, filename = "fennel.fnl", moduleName = module_name, scope = specials.compilerScope, useMetadata = true})
  for k, v in pairs(built_ins) do
    compiler.globalScope.macros[k] = v
  end
  compiler.globalScope.macros["\206\187"] = compiler.globalScope.macros.lambda
